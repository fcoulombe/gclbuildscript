...
class SoundPipeline(Pipeline.Pipeline):
    def __init__(self):
    ...
    def ProcessData(self, env, target):
        ...
        env.KAlias("@build_sounds", tgtList)
        return tgtList

       
    def generate(seld, env):
        soundBld =Builder(action = Action(Pipeline.builder_copy, cmdstr='[sound] $TARGET'), suffix='.tex') #,  src_suffix='.x'
        env.Append(BUILDERS = {'SoundBuilder' :  soundBld})
